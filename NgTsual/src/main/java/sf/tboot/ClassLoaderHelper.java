...
class ClassLoaderHelper
{
	...
	private static final Method find0(final String name, final Class... classes)
	{
		return find0(ClassLoader.class, name, classes);
	}

	private static final Method find0(final Class clazz, final String name, final Class... classes)
	{
	...
	static final void resolveClass(ClassLoader loader, Class<?> c)
	{
		try {
	...
	static final Vector<Class<?>> getFieldClasses(final ClassLoader loader)
	{
		return AccessController.doPrivileged(new PrivilegedAction<Vector<Class<?>>>()
		{
			...
			{
				try {
					final Field classes = ClassLoader.class.getDeclaredField("classes");
